...
const RowRenderer = ({ rowClass, size, start, style, depth, href, target }, cells) => {
  if (href) {
    return (index.h("a", { href: href, target: target, class: `rgRow ${rowClass || ''}`, style: Object.assign(Object.assign({}, style), { height: `${size}px`, transform: `translateY(${start}px)`, paddingLeft: depth ? `${PADDING_DEPTH * depth}px` : undefined }) }, cells));
  }
...
const RevogrData = class {
  ...
  render() {
    ...
    for (let rgRow of rows) {
      ...
      rowsEls.push(index.h(RowRenderer, { rowClass: rowClass, size: rgRow.size, start: rgRow.start, target: this.target, href: dataRow.href }, cells));
    }
    return rowsEls;
  }
